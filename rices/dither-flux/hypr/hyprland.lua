-- ~/archenemy/rices/dither-flux/hypr/hyprland.lua
...
hl.config({
    general = {
        ...
        -- dither-flux: ramka 1 px w kolorze piany na aktywnym oknie, woda na
        -- nieaktywnych. Bez gradientu — raster i gradient to sprzeczne materiały.
        ["col.active_border"]   = "rgb(D8E6EE)",
        ["col.inactive_border"] = "rgba(5C87A3AA)",
    ...
    decoration = {
        -- dither-flux: ZERO zaokrągleń. Zaokrąglony, antyaliasowany róg walczy
        -- z rastrem tapety; ostry róg jest z nim z jednego materiału.
        rounding = 0,

        active_opacity   = 1.0,
        inactive_opacity = 1.0,

        -- Blur wyłączony świadomie: rozmycie i dither to sprzeczne materiały —
        -- raster pod rozmytym szkłem zamienia się w szarą kaszę. Wyspy waybara,
        -- rofi i mako mają w tym ricie tła niemal nieprzezroczyste, więc blur nie
        -- ma czego rozmywać. Parametry zostają na wypadek powrotu.
        blur = {
            enabled           = false,
            size              = 7,
            passes            = 3,
        ...
        -- Cień wyłączony: Hyprland rysuje wyłącznie cień rozmyty, a miękka
        -- poświata pod ostrą ramką wygląda jak błąd renderowania rastra.
        shadow = {
            enabled        = false,
...
-- Layer rules — bez blur/ignore_alpha (blur w tym ricu wyłączony, patrz
-- decoration); zostają wyłącznie animacje warstw.
