...
MONITOR_CONF="$ARCHENEMY_DIR/config/hypr/monitorshyprl.conf"
...
# 8a. Monitory (rozdzielczość/pozycja/skala)
{
    ...
    for mon in "${MONITOR_NAMES[@]}"; do
        ...
        if [[ -n "$rate" ]]; then
            echo "monitor = $mon, ${res}@${rate}, ${pos}, ${scale}"
        else
            echo "monitor = $mon, ${res}, ${pos}, ${scale}"
        fi
    done
    echo "monitor = , preferred, auto, 1"
} > "$MONITOR_CONF"
echo -e "  ${GREEN}✓ monitorshyprl.conf${NC}"
...
# Bindy Super+1..0 i scroll też zależą od trybu, więc SĄ GENEROWANE tutaj
...
generate_workspaces_monitors "$HYPR_LOCAL_DIR/workspaces-monitors.conf" "$WS_MODE" "${ORDERED_LR[@]}"
echo -e "  ${GREEN}✓ workspaces-monitors.conf (tryb: $WS_MODE, ${#ORDERED_LR[@]} monitors)${NC}"

# 8c. Bindy sprzętowe (klawisze ASUS ROG tylko na ASUS-ach)
{
    echo "# generated by archenemy install.sh"
    if [[ "$HW_PROFILE" == "asus" ]]; then
        echo "# Górne klawisze ASUS ROG"
        echo "bind = , XF86Launch1, exec, rog-control-center"
        echo "bind = , XF86Launch4, exec, asusctl profile -n"
    else
        echo "# Brak bindów sprzętowych dla tego profilu ($HW_PROFILE)"
    fi
} > "$HYPR_LOCAL_DIR/hardware-keys.conf"
echo -e "  ${GREEN}✓ hardware-keys.conf ($HW_PROFILE)${NC}"
...
} > "$HYPR_LOCAL_DIR/gpu-env.conf"
echo -e "  ${GREEN}✓ gpu-env.conf ($GPU_KIND)${NC}"

# 8e. Autostart osobisty (nie nadpisuj, jeśli użytkownik już ma swój)
AUTOSTART_CONF="$HYPR_LOCAL_DIR/autostartpersonalisation.conf"
if [[ ! -f "$AUTOSTART_CONF" ]]; then
    {
        echo "# generated by archenemy install.sh"
        echo "# Osobisty autostart tej maszyny — dopisz swoje aplikacje."
        if [[ "$HW_PROFILE" == "asus" ]]; then
            echo "exec-once = rog-control-center"
        fi
        echo "# exec-once = megasync"
    } > "$AUTOSTART_CONF"
    echo -e "  ${GREEN}✓ autostartpersonalisation.conf (nowy)${NC}"
else
    echo -e "  ${YELLOW}⚠ autostartpersonalisation.conf istnieje — nie ruszam.${NC}"
fi

# 8f. Bindy aplikacji użytkownika (edycja przez Super+A — nie nadpisuj)
APPBINDS_CONF="$HYPR_LOCAL_DIR/appbinds.conf"
if [[ ! -f "$APPBINDS_CONF" ]]; then
    {
        echo "# generated by archenemy install.sh"
        echo "# Your app shortcuts — edit via Super+A (scripts/appbinds/appbinds.sh)."
    } > "$APPBINDS_CONF"
    echo -e "  ${GREEN}✓ appbinds.conf (nowy)${NC}"
else
    echo -e "  ${YELLOW}⚠ appbinds.conf istnieje — nie ruszam.${NC}"
fi
