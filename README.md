...
│   │                                 #   networkmanager-dmenu from white-blue (symlink).
│   └── crt/                          # Blue/cyan phosphor CRT: a STATIC screen shader
│                                     #   (hypr/shaders/crt.frag — scanlines, aperture mask,
│                                     #   vignette; toggle in Super+A → [c]), glass islands with
│                                     #   glow, 4px corners, amber #FFB000 for alarms only. Full
│                                     #   own layer set; networkmanager-dmenu from white-blue (symlink).
├── scripts/
...
│   │                                 #   [c] screen shader on/off (rices with one — crt);
...
│                                     #   dither-orb-spinnin' / dither-orb-static (NCS-orb-style visualizer —
│                                     #   spectrum, bass, beat; rotating / still), dither-scope (oscilloscope,
│                                     #   waveform with trigger), dither-scope-xy (Lissajous from L/R);
│                                     #   candidates 2026-09-17: dither-bars, -waterfall, -rings, -radar, -tunnel.
...
**Music visualization.** Regular animations do NOT react to sound (2026-09-08 decision: reactivity only where the animation is built for it). An animation whose shader declares `#pragma flux audio 1` is a **music visualization**: flux-wall starts listening for it on its own — **only the output monitor, never the microphone** — and tags it `♪ music visualisation` in `Super+W`. No toggle: it turns on with the animation and off with it. Today: `dither-orb-spinnin'` and `dither-orb-static` (the NCS-style orb, rotating / still — the twin differs in one constant, `ORB_SPIN`), `dither-scope` (oscilloscope: the triggered waveform, like a real scope's Y-t mode) and `dither-scope-xy` (Lissajous figures from the L/R channels — "oscilloscope music"). The animation name may contain an apostrophe (`dither-orb-spinnin'`); `install.sh` [9.6] migrates an old `dither-orb` choice to the new name. **Candidates from 2026-09-17** (built for the `crt` rice by parallel agents, awaiting the owner's live pick — any of them may be dropped): `dither-bars` (mirrored spectrum bars with a real peak-hold line), `dither-waterfall` (a sonar-style spectrogram scrolling down), `dither-rings` (phosphor rings fired from the centre on every beat, rippled by the mids), `dither-radar` (a PPI radar sweep whose echoes are the spectrum — bass near the hub, highs at the rim), `dither-tunnel` (a perspective tunnel of spectrum rings, Winamp-style). Underneath is `audio.c` (libpulse via pipewire-pulse, no cava): every 20 ms a 2048-point FFT with a Hann window, auto-gain with a floor, noise gate, smoothing, beat detector; shaders receive `audio_level/bass/lowmid/mid/high/beat`, `sampler2D audio_spectrum` (32 log bins) and `sampler2D audio_wave` (1024 stereo samples, r = L, g = R, in -1..1, captured after a rising-zero-crossing **trigger** computed in C so a periodic tone stands still like on a real scope; capture is stereo since 2026-09-17, bands come from the L+R mix), and the engine can additionally warp time (`#pragma flux audio_tempo`) and pulse brightness (`audio_glow`) — trails fade over real seconds, so music changes the motion, not the character. A missing audio server is never an error. Debugging without a server: `--audio-file file.f32`; forced off: `--no-audio`.
...
| `crt` | Blue/cyan phosphor CRT — the owner's six `hyprpicker` samples: `#043D7B`/`#0B358B` (glass, plates), `#3069AD` (borders, dim text), `#5695D6` (text), `#36D2D8` (accent, active), `#74DCE2` (bright text); the dark base `#031533` is `#043D7B` darkened in the same hue (the palette has no near-black), and amber `#FFB000` — classic amber phosphor — is the only warm color, reserved for alarms. The heart of the rice is a **static screen shader** (`rices/crt/hypr/shaders/crt.frag`, applied via `decoration.screen_shader`): scanlines every 3 physical px, a faint aperture-grille mask, a vignette and a slight phosphor lift of blacks — deliberately **without the `time` uniform**, so Hyprland's damage tracking stays on and the shader costs one pass over the redrawn area, not the whole screen every frame. Toggle: `Super+A` → `[c]` (`data/screen-shader.dat`, `hyprctl reload`). Glass islands with blur (alpha 0.55) and a cyan glow, 4px corners, 1px gradient borders (`#36D2D8` → `#5695D6`), phosphor glow (`shadow`) around the active window. Full own set: waybar (launcher glyph as a power LED), rofi, alacritty (cool ANSI, still distinguishable), mako, swayosd, hyprlock (glowing clock via label shadow), fastfetch, MangoHud, nvim (hand-tuned colorscheme); networkmanager-dmenu from `white-blue` via symlink. Wallpaper: the `crt` oscilloscope set from `gen_crt_wallpaper.py`; flux-wall animations available in the rice's palette (off by default). |
